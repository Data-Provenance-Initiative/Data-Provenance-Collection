...
def prep_summaries_for_visualization(
    ...
    df_text = df_text[df_text["Collection"].isin(
        collection_to_terms_mapper.keys())]
    df_text["Data Terms"] = df_text["Collection"].apply(
        lambda x: collection_to_terms_mapper[x])
    ...
    df["Source Category"] = df.apply(
        lambda row: [x for x in row["Source Category"] if x != "Unsure"], axis=1)
...
def plot_stacked_creator_categories(
    ...
    df_grouped = df_categories.groupby(
        ['Modality', "Creator Categories"]).size().reset_index(name='count')

    # Add percentage column based on counts
    df_grouped['percentage'] = df_grouped.groupby(
        'Modality')['count'].transform(lambda x: (x / x.sum()) * 100)
    ...
    df_grouped['midpoints'] = df_grouped.groupby(
        'Modality')['percentage'].transform(calculate_midpoints)
    ...
    if save_dir:
        chart_categories.save(os.path.join(
            save_dir, "creator_categories_by_modality.svg"), format='svg')
...
def map_country_to_continent(df):
    ...
    df_countries = df_countries[["Continent",
                                 "Modality", "Total Tokens", "Hours"]]
    df_countries['Total Hours'] = df_countries.apply(
        lambda row: row['Total Tokens'] if row['Modality'] == 'Text' else row['Hours'], axis=1)
    ...
    # print(df_countries)
    percent_df = pd.DataFrame(0, index=df_countries["Modality"].unique(
    ), columns=df_countries["Continent"].unique())
    percent_df_hrs = pd.DataFrame(0, index=df_countries["Modality"].unique(
    ), columns=df_countries["Continent"].unique())
    ...
    for _, row in df_countries.iterrows():
        ...
        percent_df_hrs.at[modality, continent] = round(
            100 * dims / modality_total_hrs[modality], 1)

    # Convert to LaTeX
    modalities_order = ["Text", "Speech", "Video"]
    continents_order = ["Africa", "Asia", "Europe",
                        "North America", "Oceania", "South America"]
    percent_df_ordered = percent_df.reindex(
        index=modalities_order, columns=continents_order)
    latex_table_ordered = percent_df_ordered.to_latex(float_format="%.1f")

    percent_df_hrs_ordered = percent_df_hrs.reindex(
        index=modalities_order, columns=continents_order)
...
def generate_latex_licenses_terms_v2(dfx):
    ...
    def create_latex_table(df_modality):
        ...
        for license in licenses:
            ...
            for term in terms:
                value = df_modality[(df_modality['License'] == license) & (
                    df_modality['Terms'] == term)]['percentage'].values
                # print(value)
        ...
        for term in terms:
            total = df_modality[df_modality['Terms']
                                == term]['percentage'].sum()
    ...
    df[['License', 'Terms']] = df['License | Terms'].str.split(
        ' \| ', expand=True)
...
def plot_license_terms_stacked_bar_chart_collections(
    ...
    if split_text_mod:
        ...
        df = pd.concat([df_non_text, df_text_datasets,
                       df_text_collections], ignore_index=True)
    ...
    if pct_by_tokens:
        ...
        df_grouped['percentage'] = df_grouped.groupby(
            'Modality')['quantity'].transform(lambda x: (x / x.sum()) * 100)
        df_melted = df_grouped.melt(id_vars=['Modality', license_key, 'percentage'], value_vars=[
                                    'quantity'], var_name='metric', value_name='value')
    else:
        df_grouped = df.groupby(
            ['Modality', license_key]).size().reset_index(name='count')
        df_grouped['percentage'] = df_grouped.groupby(
            'Modality')['count'].transform(lambda x: (x / x.sum()) * 100)
        df_melted = df_grouped.melt(id_vars=['Modality', license_key, 'percentage'], value_vars=[
                                    'count'], var_name='metric', value_name='value')
    ...
    def calculate_midpoints(points):
        ...
        return midpoints
    df_melted['midpoints'] = df_melted.groupby(
        'Modality')['percentage'].transform(calculate_midpoints)
    ...
    if save_dir:
        chart.save(os.path.join(
            save_dir, "license_use_by_modality_collections.svg"), format='svg')
...
def generate_multimodal_license_terms_latex(df):
    # print(df)
    dfx = df.groupby(['Modality', 'License Type', 'Data Terms']
    ...
    for modality in modalities:
        ...
        terms_info = ["Model Closed", "Source Closed",
        ...
        unspec_or_open = counts_dict["Commercial"].get("Unrestricted", 0) + counts_dict["Commercial"].get(
            "Unspecified", 0) + counts_dict["Unspecified"].get("Unrestricted", 0) + counts_dict["Unspecified"].get("Unspecified", 0)
...
def plot_temporal_cumulative_sources(
    ...
    def prep_df(df, modality, top_n, cumulative_measurement, earliest_year=2015):
        ...
        if earliest_year > 2013:
            ...
            df_modsourceyears['Year Released Category'].replace(
                rep_map, inplace=True)
        ...
        df_modsourcecumulativeyears = df_modsourcecumulativeyears.sort_values(
            by="Year Released Category")
        # Assuming your dataframe is named df
        df_modsourcecumulativeyears = df_modsourcecumulativeyears[
            df_modsourcecumulativeyears['Source Category'] != "Other"]
    ...
    df_final_values = df_cumyears.groupby('Source Category').apply(
        lambda x: x.loc[x['Year Released Category'].idxmax()])
...
def prepare_speech_for_gini(df):
    ...
    df_speechlanguagesn = df_speechlanguagesn.sort_values(
        by="Year Released Category")
...
def prepare_geo_gini_data(df):
    ...
    df_spec_locs = pd.concat([df_text_locs, df_speech_locs, df_video_locs])[
        ["Unique Dataset Identifier", "Countries", "Dimension", "Modality", "Year Released Category"]]
