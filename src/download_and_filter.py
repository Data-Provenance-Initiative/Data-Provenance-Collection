...
from helpers import constants, filters, io
...
def get_collection_to_uid_and_filter_ids(data_summary):
    ...
    def extract_uids_and_filter_ids(group):
        return [
            (row["Unique Dataset Identifier"], row["Dataset Filter IDs"])
            for _, row in group.iterrows()
        ]

    collection_to_keys = dict(
        filter_df_info.groupby("Collection").apply(extract_uids_and_filter_ids)
    )
...
def main(args):
    ...
    data_summary = filters.map_license_criteria(data_summary, ALL_CONSTANTS)
    ...
    print(
        f"{n_datasets} datasets from {len(n_collections)} collections after filtering."
    )
    ...
    data_bibtex.generate_bibtex(
        filtered_data_summary, save_to_file=True, output_dir=args.savedir
    )
    ...
    for collection_key, uid_task_keys in collection_to_keys.items():
        ...
        try:
            ...
            print(
                f"You are not authorized to download Collection: {collection_key}. Please go to their HF Page and accept the dataset terms and conditions."
            )
...
if __name__ == "__main__":
    ...
    parser.add(
        ...
        required=False,
        default=None,
        choices=list(COLLECTION_FN_MAPPER.keys()),
    ...
    parser.add(
        "-od",
        "--dpi-undefined-license-override",
        required=False,
        default=0,
        choices=["0", "1"],
